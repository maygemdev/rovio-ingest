...
| `druid.overwrite_append` | Special feature which is applicable for `SaveMode.Append` only. Allows you to override previously appended data within specifiend `druid.overwrite_append_partition_num_start`/`druid.overwrite_append_partition_num_end` range | `false`   |
| `druid.overwrite_append_partition_num_start` | Required property if `druid.overwrite_append` is set to `true`. Specifies start number for segment partition num to append data withing single interval. Must be a positive integer |    |
| `druid.overwrite_append_partition_num_end` | Required property if `druid.overwrite_append` is set to `true`. Specifies last number for segment partition num to append data withing single interval. Must be a positive integer |    |
